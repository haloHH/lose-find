<template>
 <div>
     <send-card title="发布招领"></send-card>
  ...
  <el-form-item label="拾物时间" required>
    ...
    <el-form-item label="拾物人" prop="contact">
  ...
  <el-form-item>
    <el-button type="primary" @click="submitForm('ruleForm')" class="btn">立即创建</el-button>
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const res = this.$http.post('/finditemes', this.ruleForm)
          this.$router.push('/')
          this.$message({
            type: 'success',
            message: '保存成功'
